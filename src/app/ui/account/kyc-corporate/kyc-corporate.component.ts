...
@Component({
  selector: 'app-kyc-corporate',
  templateUrl: './kyc-corporate.component.html',
  styleUrls: ['./kyc-corporate.component.scss']
})
export class KycCorporateComponent implements OnInit {
  ...
  async companyFiles(event, text) {
    if (text == "certifiacate") {
      ...
      console.log('filetype>>>>>>>>>>>>>>>',this.fileToUpload4.type)
    ...
    if (text == "listOfBoards") {
      ...
      if (this.fileToUpload4.type == "image/pdf" || this.fileToUpload4.type == "image/doc" || this.fileToUpload5.type == "image/png" || this.fileToUpload5.type == "image/jpg" || this.fileToUpload5.type == "image/jpeg") {
        let reader = new FileReader();
  ...
  async kycCorporateNextSubmit() {
    if (this.kycCorporateNextForm.invalid) return
    ...
    this.uploadData.append('companyRegistryInformation', JSON.stringify(this.obj));
    this.uploadData.append('companyOfficeInformation', JSON.stringify(this.obj));
    this.uploadData.append('user_id', this.localStoreData['userId']);
    this.uploadData.append('gender', this.kycCorporateForm.value.gender);
    this.uploadData.append('firstName', this.kycCorporateForm.value.firstName);
    this.uploadData.append('lastName', this.kycCorporateForm.value.lastName);
    this.uploadData.append('corporateName', this.kycCorporateForm.value.corporateName);
    this.uploadData.append('corporationWebSite', this.kycCorporateForm.value.corporateWebsite);
    this.uploadData.append('sourceOfInvestmentFund', this.kycCorporateForm.value.sourceOfFund);
    this.uploadData.append('taxIdentificationNumber', this.kycCorporateForm.value.taxiDentification);
    this.uploadData.append('applicantDesignation', this.kycCorporateForm.value.applicantDesignation);
    try {
      this.spinner.show();
      await this.kycService.corporateKyc(this.uploadData)
